...
public class CSharpClientCodegen extends AbstractCSharpCodegen {
	...
	// TODO: v5.0 is PCL, not netstandard version 1.3, and not a specific .NET Framework.
	// This needs to be updated,
	// especially because it will conflict with .NET Framework 5.0 when released, and PCL
	// 5 refers to Framework 4.0.
	// We should support either NETSTANDARD, PCL, or Both… but the concepts shouldn't be
	// mixed.
	private static final String NETSTANDARD = "v5.0";

	private static final String UWP = "uwp";

	// Defines the sdk option for targeted frameworks, which differs from targetFramework
	// and targetFrameworkNuget
	...
	public CSharpClientCodegen() {
		...
		// NOTE: This will reduce visibility of all public members in templates. Users can
	...
	public void processOpts() {
		 ...
		 * and write it back as a boolean. This avoids oddities where additionalProperties
		 * contains "false" rather than false, which will cause the templating engine to
		 * behave unexpectedly.
		 *
		...
		if (NET35.equals(this.targetFramework)) {
			...
			// TODO: NETSTANDARD here is misrepresenting a PCL v5.0 which supports .NET
			// Framework 4.6+, .NET Core 1.0, and Windows Universal 10.0
			...
			// TODO: NETSTANDARD=v5.0 and targetFrameworkNuget=netstandard1.3. These need
			// to sync.
		...
		// Only write out test related files if excludeTests is unset or explicitly set to
		// false (see start of this method)
		if (Boolean.FALSE.equals(excludeTests)) {
			...
			if (NET40.equals(this.targetFramework)) {
				// Include minimal tests for modifications made to JsonSubTypes, since
				// code is quite different for .net 4.0 from original implementation
		...
		if (optionalProjectFileFlag) {
			...
			if (Boolean.FALSE.equals(excludeTests)) {
				// NOTE: This exists here rather than previous excludeTests block because
				// the test project is considered an optional project file.
				if (Boolean.TRUE.equals(this.netCoreProjectFileFlag)) {
	...
	public CodegenModel fromModel(String name, Schema schema, Map<String, Schema> allDefinitions) {
		...
		if (codegenModel != null && codegenModel.readWriteVars != null && codegenModel.readWriteVars.size() > 1) {
			...
			for (int i = length; i > (length / 2); i--) {
				...
				// If the property at current index is found earlier in the list, remove
				// this last instance.
	 ...
	 * does not support this syntax directly so we need to convert the pattern to a .NET
	 * compatible format and apply modifiers in a compatible way. See
	 * https://msdn.microsoft.com/en-us/library/yd1hzczs(v=vs.110).aspx for .NET options.
	 * See https://github.com/swagger-api/swagger-codegen/pull/2794 for Python's initial
	 * implementation from which this is copied.
	 */
