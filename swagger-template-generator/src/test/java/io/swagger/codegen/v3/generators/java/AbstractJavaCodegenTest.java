...
public class AbstractJavaCodegenTest {
	...
	public void preprocessSwaggerWithFormParamsSetsContentType() {
		PathItem dummyPath = new PathItem().post(new Operation().requestBody(new RequestBody())).get(new Operation());
		...
		// TODO:
		// Assert.assertEquals(openAPI.getPath("dummy").getPost().getVendorExtensions().get("x-contentType"),
	...
	public void preprocessSwaggerWithBodyParamsSetsContentType() {
		...
		Assert.assertNull(openAPI.getPaths().get("dummy").getGet().getExtensions().get("x-contentType"));
		Assert.assertEquals(openAPI.getPaths().get("dummy").getPost().getExtensions().get("x-contentType"),
				"application/json");
	}
	...
	public void testFixUpParentAndInterfaces_propertyNameDifferent_getterSetterSame_typeDifferent() {
		...
		childValueProperty1.datatype = "different_type"; // different to parent
															// "value_type"
