...
/**
 * A class that contains all codegen configuration properties a user would want to
 * manipulate. An instance could be created by deserializing a JSON file or being
 * populated from CLI or Maven plugin parameters. It also has a convenience method for
 * creating a ClientOptInput class which is THE object DefaultGenerator.java needs to
 * generate code.
 */
public class CodegenConfigurator implements Serializable {
	...
	private final Map<String, Object> dynamicProperties = new HashMap<String, Object>(); // the
																							// map
																							// that
																							// holds
																							// the
	...
	public ClientOptInput toClientOptInput() {
		...
		if (!StringUtils.isBlank(inputSpec)) {
			...
			if (config.needsUnflattenedSpec()) {
				...
				SwaggerParseResult resultUnflattened = new OpenAPIParser().readContents(inputSpec, authorizationValues,
						optionsUnflattened);
			...
			ParseOptions options = buildParseOptions();
			SwaggerParseResult result = new OpenAPIParser().readLocation(sanitizedSpecificationUrl, authorizationValues,
					options);
			...
			if (config.needsUnflattenedSpec()) {
				ParseOptions optionsUnflattened = new ParseOptions();
				optionsUnflattened.setResolve(true);
				SwaggerParseResult resultUnflattened = new OpenAPIParser().readLocation(sanitizedSpecificationUrl,
						authorizationValues, optionsUnflattened);
				OpenAPI openAPIUnflattened = resultUnflattened.getOpenAPI();
				config.setUnflattenedOpenAPI(openAPIUnflattened);
			}
