...
public class CodegenConfigLoader {
	...
	/**
	 * Tries to load config class with SPI first, then with class name directly from
	 * classpath
	...
	public static CodegenConfig forName(String name) {
		...
		try {
			...
			throw new RuntimeException(
					"Can't load config class with name ".concat(name) + " Available: " + availableConfigs.toString(),
					e);
		}
	}
