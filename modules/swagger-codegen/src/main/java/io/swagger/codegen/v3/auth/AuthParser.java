	...
	public static List<AuthorizationValue> parse(String urlEncodedAuthStr) {
		...
		if (isNotEmpty(urlEncodedAuthStr)) {
			...
			for (String part : parts) {
				...
				if (kvPair.length == 2) {
					// FIXME replace the deprecated method by decode(string, encoding).
					// Which encoding is used ? Default UTF-8 ?
					auths.add(new AuthorizationValue(URLDecoder.decode(kvPair[0]), URLDecoder.decode(kvPair[1]),
							"header"));
				}
			}
		}
