...
public class ExampleGenerator {
	...
	public List<Map<String, String>> generate(Map<String, Object> examples, List<String> mediaTypes, Schema property) {
		...
		if (examples == null) {
			if (mediaTypes == null) {
				...
				mediaTypes = Collections.singletonList(MIME_TYPE_JSON); // FIXME: a
																		// parameter
																		// should not be
																		// assigned. Also
																		// declare the
			...
			for (String mediaType : mediaTypes) {
				...
				if (modelName != null && mediaType.startsWith(MIME_TYPE_JSON)) {
					...
					if (schema != null) {
						...
						String example = Json
								.pretty(resolveModelToExample(modelName, mediaType, schema, processedModels));
					...
					final Schema schema = this.examples.get(modelName);
					String example = new XmlExampleGenerator(this.examples).toXml(schema, 0,
	...
	private Object resolvePropertyToExample(String propertyName, String mediaType, Schema property,
		...
		if (property.getExample() != null) {
			...
			if (innerType != null) {
				int arrayLength = null == ((ArraySchema) property).getMaxItems() ? 2
						: ((ArraySchema) property).getMaxItems();
				// max 10 examples to avoid OOM
				if (arrayLength > 10) {
					logger.warn("value of maxItems of property {} is {}; limiting to 10 examples", property,
							arrayLength);
					arrayLength = 10;
				}
		...
		else if (property instanceof MapSchema && property.getAdditionalProperties() != null
				&& property.getAdditionalProperties() instanceof Schema) {
			Map<String, Object> mp = new HashMap<String, Object>();
			if (property.getName() != null) {
				mp.put(property.getName(), resolvePropertyToExample(propertyName, mediaType,
						(Schema) property.getAdditionalProperties(), processedModels));
			}
			else {
				mp.put("key", resolvePropertyToExample(propertyName, mediaType,
						(Schema) property.getAdditionalProperties(), processedModels));
			}
			return mp;
		}
		...
		/**
		 * this concep no longer exists else if (property instanceof RefProperty) { String
		 * simpleName = ((RefProperty) property).getSimpleRef(); logger.debug("Ref
		 * property, simple name: {}", simpleName); Model model =
		 * examples.get(simpleName); if (model != null) { return
		 * resolveModelToExample(simpleName, mediaType, model, processedModels); }
	...
	private Object resolveModelToExample(String name, String mediaType, Schema schema, Set<String> processedModels) {
		...
		if (schema.getExample() != null) {
			...
			for (Object propertyName : schema.getProperties().keySet()) {
				...
				values.put(propertyName.toString(),
						resolvePropertyToExample(propertyName.toString(), mediaType, schema, processedModels));
			}
