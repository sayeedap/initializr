...
public class ModelUtils {
	...
	public static Operation[] createOperationArray(PathItem pathItem) {
		return Stream
				.of(pathItem.getGet(), pathItem.getPost(), pathItem.getDelete(), pathItem.getHead(), pathItem.getPut(),
						pathItem.getPatch(), pathItem.getOptions())
				.filter(Objects::nonNull).collect(Collectors.toSet()).toArray(new Operation[] {});
	}
	...
	public static void processCodegenModels(Map<String, CodegenModel> allModels) {
		...
		for (String name : allModels.keySet()) {
			...
			// TODO Determine what to do if the parent discriminator name == the
			// grandparent discriminator name
			while (parent != null) {
	...
	private static String toEnumValue(String value, String datatype) {
		if ("number".equalsIgnoreCase(datatype)) {
			...
			value = StringEscapeUtils.unescapeJava(StringEscapeUtils.escapeJava(value).replace("\\/", "/"))
					.replaceAll("[\\t\\n\\r]", " ").replace("\\", "\\\\").replace("\"", "\\\"");
