...
public class IgnoreLineParser {
	...
	enum Token {
		...
		MATCH_ALL("**"), MATCH_ANY("*"), ESCAPED_EXCLAMATION("\\!"), ESCAPED_SPACE("\\ "), PATH_DELIM("/"), NEGATE("!"),
		TEXT(null), DIRECTORY_MARKER("/"), ROOTED_MARKER("/"), COMMENT(null);
		...
		for (int i = 0, totalLength = characters.length; i < totalLength; i++) {
			...
			if (i == 0) {
				if ("#".equals(current)) {
					...
					// NOTE: Just push forward and drop the escape character. Falls
					// through to TEXT token.
					current = next;
					next = null;
					i++;
				}
			...
			if (Token.MATCH_ANY.pattern.equals(current)) {
				...
				if (Token.MATCH_ANY.pattern.equals(next)) {
					// peek ahead for invalid pattern. Slightly inefficient, but
					// acceptable.
					...
					if (sb.length() > 0) {
						// A MATCH_ANY may commonly follow a filename or some other
						// character. Dump that to results before the MATCH_ANY.
			...
			if (Token.PATH_DELIM.pattern.equals(current)) {
				if (i != totalLength - 1) {
					...
					if (Token.PATH_DELIM.pattern.equals(next)) {
						// ignore doubled path delims. NOTE: doesn't do full lookahead, so
						// /// will result in //
						i++;
					}
					continue;
