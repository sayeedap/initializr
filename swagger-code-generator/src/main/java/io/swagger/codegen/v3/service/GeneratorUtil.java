...
public class GeneratorUtil {
	...
	public static io.swagger.codegen.ClientOptInput getClientOptInputV2(GenerationRequest generationRequest) {
		...
		if (!generationRequest.getOptions().getAllowedAuthHosts().isEmpty()
				|| !generationRequest.getOptions().getDeniedAuthHosts().isEmpty()) {
			urlMatcher = url -> {
		...
		final List<io.swagger.models.auth.AuthorizationValue> authorizationValues = io.swagger.codegen.auth.AuthParser
				.parse(generationRequest.getOptions().getAuth());
		...
		if (generationRequest.getOptions().getAuthorizationValue() != null) {
			...
			if (generationRequest.getOptions().getAuthorizationValue().getUrlMatcher() != null) {
				predicateUrlMatcher = url -> generationRequest.getOptions().getAuthorizationValue().getUrlMatcher()
						.test(url);
			}
		...
		if (codegenConfig.isUsingFlattenSpec()
				&& !Boolean.FALSE.equals(generationRequest.getOptions().isUsingFlattenSpecForV2())) {
		...
		if (StringUtils.isBlank(inputSpec)) {
			if (inputSpecURL != null) {
				if (!authorizationValues.isEmpty()) {
					swagger = new SwaggerParser().read(inputSpecURL, authorizationValues, parseOptions);
				}
				else {
	...
	public static void validateSpec(String lang, String inputSpec, String inputSpecURL) {
		...
		if ((StringUtils.isBlank(inputSpec) || "{}".equals(inputSpec) || "{ }".equals(inputSpec))
				&& StringUtils.isBlank(inputSpecURL)) {
	...
	private static void readCodegenArguments(CodegenConfigurator configurator, Options options) {
		...
		io.swagger.codegen.v3.CodegenConfig config = io.swagger.codegen.v3.CodegenConfigLoader
				.forName(configurator.getLang());
		if (config == null) {
			return;
		}
