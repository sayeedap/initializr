...
public class HandlebarTemplateEngine implements TemplateEngine {
	...
	private com.github.jknack.handlebars.Template getHandlebars(String templateFile) throws IOException {
		...
		String customTemplateDir = config.customTemplateDir() != null ? config.customTemplateDir().replace("\\", "/")
				: null;
		templateLoader = new CodegenTemplateLoader().templateDir(templateDir).customTemplateDir(customTemplateDir);
