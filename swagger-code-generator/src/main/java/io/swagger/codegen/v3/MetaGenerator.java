...
/**
 * @deprecated use instead {@link DefaultGenerator} or cli interface from
...
public class MetaGenerator extends AbstractGenerator {
	...
	protected void generate(String[] args) {
		...
		File sourceFolder = new File(
				outputFolder + File.separator + "src/main/java/" + targetPackage.replace('.', File.separatorChar));
		...
		supportingFiles
				.add(new SupportingFile("api.template", "src/main/resources" + File.separator + name, "api.mustache"));
		supportingFiles.add(
				new SupportingFile("model.template", "src/main/resources" + File.separator + name, "model.mustache"));

		supportingFiles.add(new SupportingFile("services.mustache", "src/main/resources/META-INF/services",
