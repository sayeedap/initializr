...
public abstract class Rule {
	...
	/**
	 * Whether or not the rule should be negated. !foo means foo should be removed from
	 * previous matches. Example: **\/*.bak excludes all backup. Adding !/test.bak will
	 * include test.bak in the project root.
	 * <p>
	 * NOTE: It is not possible to re-include a file if a parent directory of that file is
	 * excluded.
	 * @return {@code true} if the rule is negated (inverse), otherwise {@code false}
	 * (normal).
	 */
	...
	public static Rule create(String definition) {
		// NOTE: Comments that start with a : (e.g. //:) are pulled from git documentation
		// for .gitignore
		// see:
		...
		try {
			...
			if (result.size() == 0) {
				...
				directoryOnly = IgnoreLineParser.Token.DIRECTORY_MARKER
						.equals(result.get(result.size() - 1).getToken());
