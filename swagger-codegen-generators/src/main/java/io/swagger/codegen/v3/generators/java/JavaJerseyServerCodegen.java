...
public class JavaJerseyServerCodegen extends AbstractJavaJAXRSServerCodegen {
	 ...
	 *
	 * @see
	 * io.swagger.codegen.languages.AbstractJavaJAXRSServerCodegen#postProcessOperations(
	 * java.util.Map)
	 */
	@Override
		...
		if (useTags) {
			...
			if (operations != null) {
				...
				if (!allPaths.isEmpty()) {
					...
					partsLoop: for (String part : parts) {
					...
					if (basePath.length() > 0) {
						...
						for (CodegenOperation operation : ops) {
							operation.path = operation.path
									.substring(basePath.length() + (operation.path.startsWith("/") ? 1 : 0));
	...
	public void addOperationToGroup(String tag, String resourcePath, Operation operation, CodegenOperation co,
		...
		if (useTags) {
			// only add operations to group; base path extraction is done in
