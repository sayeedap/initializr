 ...
 * <p>
 * Generator authors may add helpers for explicitly adding prefixed spaces which fragments
 * won't be aware of.
 * <p>
 * Register: <pre>
...
public class IndentedLambda implements Lambda {
	 ...
	 * @param prefixSpaceCount The number of indented characters to apply as a prefix to a
	 * fragment.
	 * @param indentionCharacter String representation of the character used in the indent
	 * (e.g. " ", "\t", ".").
	 */
	...
	public Object apply(Object o, com.github.jknack.handlebars.Template template) throws IOException {
		...
		for (int i = 0; i < lines.length; i++) {
			String line = lines[i];
			// Mustache will apply correct indentation to the first line of a template (to
			...
			// We've split on the system's line separator. We don't want to add an
			// additional trailing line.
			if (i < lines.length - 1) {
