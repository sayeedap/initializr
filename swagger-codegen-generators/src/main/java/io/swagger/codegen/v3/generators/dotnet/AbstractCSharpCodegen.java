...
public abstract class AbstractCSharpCodegen extends DefaultCodegenConfig {
	...
	// TODO: Add option for test folder output location. Nice to allow e.g. ./test instead
	// of ./src.
	// This would require updating relative paths (e.g. path to main project file in test
	// project file)
	...
	public AbstractCSharpCodegen() {
		...
		collectionTypes = new HashSet<String>(
				Arrays.asList("IList", "List", "ICollection", "Collection", "IEnumerable"));

		mapTypes = new HashSet<String>(Arrays.asList("IDictionary"));

		setReservedWords(Arrays.asList(
				// set "client" as a reserved word to avoid conflicts with
				// IO.Swagger.Client
				// this is a workaround and can be removed if c# api client is updated to
				// use
				...
				"localVarPostBody", "localVarHttpHeaderAccepts", "localVarHttpHeaderAccept", "localVarHttpContentTypes",
				"localVarHttpContentType", "localVarStatusCode", "ApiResponse", "apiresponse",
				// C# reserved words
		...
		languageSpecificPrimitives = new HashSet<String>(
				Arrays.asList("String", "string", "bool?", "double?", "decimal?", "int?", "long?", "float?", "byte[]",
						"ICollection", "Collection", "List", "Dictionary", "DateTime?", "DateTimeOffset?", "String",
						"Boolean", "Double", "Int32", "Int64", "Float", "Guid?", "System.IO.Stream", // not
																										// really
																										// a
																										// primitive,
																										// we
	...
	public void processOpts() {
		...
		if (additionalProperties.containsKey(CodegenConstants.PRESERVE_COMMENT_NEWLINES)) {
			setPreserveNewLines(
					Boolean.valueOf(additionalProperties.get(CodegenConstants.PRESERVE_COMMENT_NEWLINES).toString()));
		}
		...
		if (additionalProperties.containsKey(CodegenConstants.INTERFACE_PREFIX)) {
			...
			if ("false".equals(useInterfacePrefix.toLowerCase())) {
				...
				// NOTE: if user passes "true" explicitly, we use the default I- prefix.
				// The other supported case here is a custom prefix.
				setInterfacePrefix(sanitizeName(useInterfacePrefix));
			}
		}

		// This either updates additionalProperties with the above fixes, or sets the
		// default if the option was not specified.
	...
	private void addHandlebarsLambdas(Map<String, Object> objs) {
		...
		Map<String, Lambda> lambdas = new ImmutableMap.Builder<String, Lambda>()
				.put("lowercase", new LowercaseLambda().generator(this)).put("uppercase", new UppercaseLambda())
				.put("titlecase", new TitlecaseLambda()).put("camelcase", new CamelCaseLambda().generator(this))
				.put("camelcase_param", new CamelCaseLambda().generator(this).escapeAsParamName(true))
				.put("indented", new IndentedLambda()).put("indented_8", new IndentedLambda(8, " "))
				.put("indented_12", new IndentedLambda(12, " ")).put("indented_16", new IndentedLambda(16, " "))
				.build();

		if (objs.containsKey("lambda")) {
			LOGGER.warn(
					"An property named 'lambda' already exists. Mustache lambdas renamed from 'lambda' to '_lambda'. "
	...
	/**
	 * Invoked by {DefaultGenerator} after all models have been post-processed, allowing
	 * for a last pass of codegen-specific model cleanup.
	...
	/**
	 * C# differs from other languages in that Enums are not _true_ objects; enums are
	 * compiled to integral types. So, in C#, an enum is considers more like a
	 * user-defined primitive.
	 *
	 * When working with enums, we can't always assume a RefModel is a nullable type
	 * (where default(YourType) == null), so this post processing runs through all models
	 * to find RefModel'd enums. Then, it runs through all vars and modifies those vars
	 * referencing RefModel'd enums to work the same as inlined enums rather than as
	 * objects.
			...
			if (model != null) {
				for (CodegenProperty var : model.allVars) {
					if (enumRefs.containsKey(var.datatype)) {
						...
						// This is different in C# than most other generators, because
						// enums in C# are compiled to integral types,
						// while enums in many other languages are true objects.
						CodegenModel refModel = enumRefs.get(var.datatype);
						var.allowableValues = new HashMap<>(refModel.allowableValues);
						updateCodegenPropertyEnum(var);

						// We do these after updateCodegenPropertyEnum to avoid
						// generalities that don't mesh with C#.
				...
				if (getBooleanValue(model, CodegenConstants.IS_ENUM_EXT_NAME)) {
					// We now need to make allowableValues.enumVars look like the context
					// of CodegenProperty
					...
					if (model.dataType.startsWith("byte")) {
						// C# Actually supports byte and short enums, swagger spec only
						// supports byte.
						isByte = true;
						...
						// C# doesn't support non-integral enums, so we need to treat
						// everything else as strings (e.g. to not lose precision or data
						// integrity)
						isString = true;
						model.vendorExtensions.put("x-enum-string", true);
					}

					// Since we iterate enumVars for modelnnerEnum and enumClass
					// templates, and CodegenModel is missing some of CodegenProperty's
					// properties,
					// we can take advantage of Mustache's contextual lookup to add the
					// same "properties" to the model's enumVars scope rather than
					// CodegenProperty's scope.
					List<Map<String, String>> enumVars = (ArrayList<Map<String, String>>) model.allowableValues
							.get("enumVars");
				...
				LOGGER.warn(
						"Expected to retrieve model %s by name, but no model was found. Check your -Dmodels inclusions.",
						swaggerName);
			}
		}
	}
	...
	public void updateCodegenPropertyEnum(CodegenProperty var) {
		...
		// Because C# uses nullable primitives for datatype, and datatype is used in
		// DefaultCodegen for determining enum-ness, guard against weirdness here.
		if (getBooleanValue(var, CodegenConstants.IS_ENUM_EXT_NAME)) {
			if ("byte".equals(var.dataFormat)) {// C# Actually supports byte and short
												// enums.
			...
			else {// C# doesn't support non-integral enums, so we need to treat everything
				...
				for (CodegenOperation operation : ops) {
					...
					if (operation.returnType != null) {
						...
						if (this.collectionTypes.contains(typeMapping)) {
							operation.getVendorExtensions().put(CodegenConstants.IS_LIST_CONTAINER_EXT_NAME,
									Boolean.TRUE);
							operation.returnContainer = operation.returnType;
							if (this.returnICollection
									&& (typeMapping.startsWith("List") || typeMapping.startsWith("Collection"))) {
								// NOTE: ICollection works for both List<T> and
								// Collection<T>
							...
							operation.returnContainer = operation.returnType;
							operation.getVendorExtensions().put(CodegenConstants.IS_MAP_CONTAINER_EXT_NAME,
									this.mapTypes.contains(typeMapping));
						}
					}
	...
	public String apiFileFolder() {
		return outputFolder + File.separator + sourceFolder + File.separator + packageName + File.separator
				+ apiPackage();
	}
	...
	public String toOperationId(String operationId) {
		// throw exception if method name is empty (should not occur as an auto-generated
	...
	protected boolean isReservedWord(String word) {
		// NOTE: This differs from super's implementation in that C# does _not_ want case
	...
	protected String getRefSchemaTargetType(Schema schema, String schemaType) {
		...
		if (schema != null && schema.get$ref() != null) {
			...
			if (refSchema != null && !isObjectSchema(refSchema)
					&& !(refSchema instanceof ArraySchema || refSchema instanceof MapSchema)
					&& refSchema.getEnum() == null) {
	...
	/**
	 * Provides C# strongly typed declaration for simple arrays of some type and arrays of
	 * arrays of some type.
	...
	private String getArrayTypeDeclaration(ArraySchema arr) {
		// TODO: collection type here should be fully qualified namespace to avoid model
		// conflicts
	...
	public String toModelName(String name) {
		// We need to check if import-mapping has a different model for this class, so we
		// use it
		...
		if (name.matches("^\\d.*")) {
			...
			name = "model_" + name; // e.g. 200Response => Model200Response (after
									// camelize)
		}
	...
	public String toEnumValue(String value, String datatype) {
		...
		// C# only supports enums as literals for int, int?, long, long?, byte, and byte?.
		// All else must be treated as strings.
		// Per:
	...
	public String toEnumVarName(String name, String datatype) {
		...
		if (NumberUtils.isNumber(name)) {
			return "NUMBER_" + name.replaceAll("-", "MINUS_").replaceAll("\\+", "PLUS_").replaceAll("\\.", "_DOT_");
		}
	...
	public void preprocessOpenAPI(OpenAPI openAPI) {
		...
		if (urlInfo != null && urlInfo.getPort() > 0) {
			additionalProperties.put("serverUrl",
					String.format("%s://%s:%s", urlInfo.getProtocol(), urlInfo.getHost(), urlInfo.getPort()));
		...
		if (this.preserveNewLines) {
			Map<String, Schema> schemaMap = openAPI.getComponents() != null ? openAPI.getComponents().getSchemas()
					: null;
			if (schemaMap != null) {
	 ...
	 *
	 * @Override public void postProcessParameter(CodegenParameter parameter) { if
	 * (parameter.getIsBinary()) { parameter.dataType = "System.IO.Stream"; }
