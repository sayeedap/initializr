...
public class JavaClientCodegenTest extends AbstractCodegenTest {
	...
	public void arraysInRequestBody() throws Exception {
		...
		body1.setContent(new Content().addMediaType("application/json",
				new MediaType().schema(new ArraySchema().items(new StringSchema()))));
		CodegenParameter codegenParameter1 = codegen.fromRequestBody(body1, null, null, new HashMap<String, Schema>(),
				new HashSet<String>());
		...
		body2.setContent(new Content().addMediaType("application/json",
				new MediaType().schema(new ArraySchema().items(new ArraySchema().items(new IntegerSchema())))));
		CodegenParameter codegenParameter2 = codegen.fromRequestBody(body2, null, null, new HashMap<String, Schema>(),
				new HashSet<String>());
		...
		body3.setContent(new Content().addMediaType("application/json", new MediaType()
				.schema(new ArraySchema().items(new ObjectSchema().$ref("#/components/schemas/Point")))));
		...
		CodegenParameter codegenParameter3 = codegen.fromRequestBody(body3, null, null,
				Collections.<String, Schema>singletonMap("Point", point), new HashSet<String>());
	...
	public void testPackageNamesSetWithSetters() throws Exception {
		...
		Assert.assertEquals(codegen.additionalProperties().get(CodegenConstants.INVOKER_PACKAGE),
				"xxx.yyyyy.zzzzzzz.invoker");
	}
	...
	public void customTemplates() throws Exception {
		...
		codegen.additionalProperties().put(CodegenConstants.TEMPLATE_DIR,
				String.join(File.separator, "user", "custom", "location"));
		codegen.processOpts();
	...
	public void testModelNamedFile() {
		...
		final CodegenModel codegenModelSetting = config.fromModel("Setting", modelSetting,
		...
		CodegenProperty fileProperty = codegenProperties.stream().filter(property -> property.name.equals("file"))
				.findAny().get();
		...
		CodegenProperty documentProperty = codegenProperties.stream()
				.filter(property -> property.name.equals("document")).findAny().get();
