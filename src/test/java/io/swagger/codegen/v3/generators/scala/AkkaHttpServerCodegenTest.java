...
public class AkkaHttpServerCodegenTest {
	...
	public void testSetComplexTypes() {
		...
		List<CodegenOperation> operationList = new LinkedList<CodegenOperation>() {
			{
				addAll(Arrays.asList(codegenOperation1, codegenOperation2, codegenOperation3, codegenOperation4));
			}
		};
		...
		Assert.assertEquals(result.get("complexRequestTypes"), new HashSet<String>() {
			{
				addAll(Arrays.asList("Pet", "User"));
			}
		});
		Assert.assertEquals(result.get("complexReturnTypes"), new LinkedList<CodegenResponse>() {
			{
				addAll(Arrays.asList(response1, response2, response3));
			}
		});
		Assert.assertEquals(codegenOperation1.getVendorExtensions().get("complexReturnTypes"),
				new LinkedList<CodegenResponse>() {
					{
						add(response1);
					}
				});
		Assert.assertEquals(codegenOperation2.getVendorExtensions().get("complexReturnTypes"),
				new LinkedList<CodegenResponse>() {
					{
						addAll(Arrays.asList(response1, response3));
					}
				});
	}
	...
	public void testAddLowercaseHttpMethod() {
		...
		Assert.assertEquals(codegenOperation.getVendorExtensions().get(AkkaHttpServerCodegen.LOWERCASE_HTTP_METHOD),
				"get");
	}
	...
	public void testAddPathMatcher() {
		...
		Assert.assertEquals(
				(LinkedList<TextOrMatcher>) codegenOperation.getVendorExtensions().get(AkkaHttpServerCodegen.PATHS),
				expectedPaths);
	...
	public void testAddQueryParamsWithSupportedTypes() {
		...
		Assert.assertEquals((LinkedList<CodegenParameter>) codegenOperation.getVendorExtensions()
				.get(AkkaHttpServerCodegen.QUERY_PARAMS_WITH_SUPPORTED_TYPE), expectedMatchedPathParams);
	}
	...
	public void testAddAllParamsWithSupportedTypes() {
		...
		Assert.assertEquals((LinkedList<CodegenParameter>) codegenOperation.getVendorExtensions()
				.get(AkkaHttpServerCodegen.PARAMS_WITH_SUPPORTED_TYPE), expectedMatchedPathParams);
	}
	...
	public void testFileGeneration() throws IOException {
		...
		final CodegenConfigurator configurator = new CodegenConfigurator().setLang("scala-akka-http-server")
