...
public class DefaultCodegenConfigTest {
	...
	public void testInitialValues() throws Exception {
		...
		Assert.assertEquals(codegen.additionalProperties().get(CodegenConstants.SORT_PARAMS_BY_REQUIRED_FLAG),
				Boolean.TRUE);
	...
	public void testNumberSchemaMinMax() {
		Schema schema = new NumberSchema().minimum(BigDecimal.valueOf(50)).maximum(BigDecimal.valueOf(1000));
	...
	public void testFromOperation_BodyParamsUnique() {
		PathItem dummyPath = new PathItem().post(new Operation()).get(new Operation());

		OpenAPI openAPI = new OpenAPI().path("dummy", dummyPath);
	...
	public void testGetCollectionFormat(Parameter.StyleEnum style, Boolean explode, String expectedCollectionFormat) {
		...
		ArraySchema paramSchema = new ArraySchema().items(new IntegerSchema());
		Parameter param = new Parameter().in("query").name("testParameter").schema(paramSchema).style(style)
				.explode(explode);
	...
	public Object[][] provideData_testGetCollectionFormat() {
		...
		return new Object[][] { { null, null, "multi" }, { Parameter.StyleEnum.FORM, null, "multi" },
				{ null, Boolean.TRUE, "multi" }, { Parameter.StyleEnum.FORM, Boolean.TRUE, "multi" },

				{ null, Boolean.FALSE, "csv" }, { Parameter.StyleEnum.FORM, Boolean.FALSE, "csv" },
	...
	/**
	 * Tests that
	 * {@link DefaultCodegenConfig#fromOperation(String, String, Operation, java.util.Map, OpenAPI)}
	 * correctly resolves the consumes list when the request body is specified via
	...
	public void testRequestBodyRefConsumesList() {
		...
		final CodegenOperation codegenOp = codegen.fromOperation(path, "post", op, openAPI.getComponents().getSchemas(),
				openAPI);
	...
	/**
	 * Tests when a 'application/x-www-form-urlencoded' request body is marked as required
	 * that all form params are also marked as required.
	 *
	...
	public void testRequiredFormParams() {
		...
		for (CodegenParameter formParam : formParams) {
			Assert.assertTrue(formParam.getRequired(),
					"Form param '" + formParam.getParamName() + "' is not required.");
		}
	...
	/**
	 * Tests when a 'application/x-www-form-urlencoded' request body is marked as optional
	 * that all form params are also marked as optional.
	 *
	...
	public void testFromResponse_inlineHeaders() {
		...
		Header inlineHeader = new Header().description("This is header1")
				.schema(new Schema().type("string").example("header_val"));
	...
	public void testFromResponse_referenceHeaders() {
		...
		OpenAPI openAPI = new OpenAPI().components(new Components().responses(new HashMap<>()));
		Header referencedHeader = new Header()
				.schema(new Schema().description("This is header1").type("string").example("header_val"));
	...
	public Object[][] provideData_testCommonPrefix() {
		return new Object[][] { { Collections.singletonList("FOO_BAR"), "" },
				{ Arrays.asList("FOO_BAR", "FOO_BAZ"), "FOO_" }, { Arrays.asList("FOO_BAR", "FOO_BAZ", "TEST"), "" },
