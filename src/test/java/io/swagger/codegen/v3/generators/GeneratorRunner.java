...
public abstract class GeneratorRunner {
	...
	public static List<File> runGenerator(String name, String specPath, GenerationRequest.CodegenVersion codegenVersion,
			boolean v2Spec, boolean yaml, boolean flattenInlineComposedSchema, String outFolder) throws Exception {
		...
		request.codegenVersion(codegenVersion) // use V2 to target Swagger/OpenAPI 2.x
												// Codegen version
				.type(GenerationRequest.Type.CLIENT).lang(name).spec(loadSpecAsNode(specPath, yaml, // YAML
																									// file,
																									// use
																									// false
																									// for
																									// JSON
						v2Spec)) // OpenAPI 3.x - use true for Swagger/OpenAPI 2.x
									// definitions
				.options(new Options().flattenInlineComposedSchema(flattenInlineComposedSchema).outputDir(path));
