...
public class KotlinServerCodegen extends AbstractKotlinCodegen {
	...
	// This is here to potentially warn the user when an option is not supoprted by the
	// target framework.
	private Map<String, List<String>> optionsSupportedPerFramework = singletonMap(Constants.KTOR,
			Arrays.asList(Constants.AUTOMATIC_HEAD_REQUESTS, Constants.CONDITIONAL_HEADERS, Constants.HSTS,
					Constants.CORS, Constants.COMPRESSION));
	...
	public KotlinServerCodegen() {
		...
		addSwitch(Constants.AUTOMATIC_HEAD_REQUESTS, Constants.AUTOMATIC_HEAD_REQUESTS_DESC,
				getAutoHeadFeatureEnabled());
		addSwitch(Constants.CONDITIONAL_HEADERS, Constants.CONDITIONAL_HEADERS_DESC,
	...
	public CodegenOperation fromOperation(String path, String httpMethod, Operation operation,
		...
		// they need to match the names in the generated data class, this is required by
		// ktor Location
	...
	public void processOpts() {
		...
		String resourcesFolder = "src/main/resources"; // not sure this can be user
														// configurable.
		Boolean generateApis = additionalProperties.containsKey(GENERATE_APIS)
				&& (Boolean) additionalProperties.get(GENERATE_APIS);
		...
		final String infrastructureFolder = (sourceFolder + File.separator + packageName + File.separator
				+ "infrastructure").replace(".", File.separator);
