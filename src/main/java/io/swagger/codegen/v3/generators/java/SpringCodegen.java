...
public class SpringCodegen extends AbstractJavaCodegen
	...
	public SpringCodegen() {
		...
		cliOptions.add(CliOption.newBoolean(DEFAULT_INTERFACES, "Generate default implementations for interfaces")
				.defaultValue("true"));
		cliOptions.add(
				CliOption.newBoolean(THROWS_EXCEPTION, "Throws Exception in operation methods").defaultValue("false"));
		cliOptions.add(CliOption.newBoolean(DATE_PATTERN, "use pattern for date parameters").defaultValue("true"));
		cliOptions.add(
				CliOption.newBoolean(DATE_TIME_PATTERN, "use pattern for date time parameters").defaultValue("true"));
	...
	public void processOpts() {
		// Process java8 option before common java ones to change the default dateLibrary
		// to java8.
			...
			if (isSpringCloudLibrary()) {
				...
				if (additionalProperties.containsKey(CodegenConstants.GENERATE_API_TESTS)) {
					if (Boolean.valueOf(additionalProperties.get(CodegenConstants.GENERATE_API_TESTS).toString())) {
						// TODO Api Tests are not currently supported in spring-cloud
						// template
						apiTestTemplateFiles.clear();
				...
				// java.io.File.separator), "ApiException.java"));
				// supportingFiles.add(new SupportingFile("apiResponseMessage.mustache",
				// (sourceFolder + File.separator + exceptionPackage).replace(".",
				// java.io.File.separator), "ApiResponseMessage.java"));
				// supportingFiles.add(new SupportingFile("notFoundException.mustache",
				// (sourceFolder + File.separator + exceptionPackage).replace(".",
				// java.io.File.separator), "NotFoundException.java"));
				...
				// supportingFiles.add(new
				// SupportingFile("LocalDateTimeConverter.mustache",
				// (sourceFolder + File.separator + configPackage).replace(".",
				// java.io.File.separator), "LocalDateTimeConverter.java"));
			}
		}
	...
	public CodegenProperty fromProperty(String name, Schema propertySchema) {
		...
		if (propertySchema != null && propertySchema.get$ref() != null) {
			...
			if (refSchema != null && !isObjectSchema(refSchema) && !(refSchema instanceof ArraySchema)
					&& !(refSchema instanceof MapSchema) && refSchema.getEnum() == null) {
	...
	/**
	 * This method removes header parameters from the list of parameters and also corrects
	 * last allParams hasMore state.
	...
	private void removeHeadersFromAllParams(List<CodegenParameter> allParams) {
		...
		allParams.get(allParams.size() - 1).getVendorExtensions().put(CodegenConstants.HAS_MORE_EXT_NAME,
				Boolean.FALSE);
	}
	...
	private void removeHeadersFromContents(List<CodegenContent> contents) {
		...
		for (int index = 0; index < contents.size(); index++) {
			...
			final List<CodegenParameter> filteredParameters = parameters.stream().filter(
					codegenParameter -> !getBooleanValue(codegenParameter, CodegenConstants.IS_HEADER_PARAM_EXT_NAME))
					.collect(Collectors.toList());
			parameters.clear();
			parameters.addAll(filteredParameters);
			parameters.get(parameters.size() - 1).getVendorExtensions().put(CodegenConstants.HAS_MORE_EXT_NAME,
					Boolean.FALSE);
		}
	}
