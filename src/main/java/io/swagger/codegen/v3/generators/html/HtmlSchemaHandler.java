...
public class HtmlSchemaHandler extends SchemaHandler {
	...
	public void processComposedSchemas(CodegenModel codegenModel, Schema schema, Map<String, CodegenModel> allModels) {
		...
		for (CodegenProperty codegenProperty : codegenProperties) {
			if (codegenProperty.getIsListContainer()) {
				Schema schemaProperty = OpenAPIUtil.getPropertyFromAllOfSchema(codegenProperty.baseName,
						composedSchema.getAllOf(), this.codegenConfig.getOpenAPI());
				if (schemaProperty == null || !(schemaProperty instanceof ArraySchema)) {
					continue;
				}
				this.processArrayItemSchema(StringUtils.EMPTY, codegenProperty, (ArraySchema) schemaProperty,
						allModels);
				break;
			}
		}
	...
	protected CodegenModel processArrayItemSchema(String codegenModelName, CodegenProperty codegenProperty,
		...
		final Schema itemsSchema = arraySchema.getItems();
		if (itemsSchema instanceof ComposedSchema) {
			this.processComposedSchema(codegenModelName, codegenProperty.items, (ComposedSchema) itemsSchema,
					allModels);
		}
		return null;
	}
