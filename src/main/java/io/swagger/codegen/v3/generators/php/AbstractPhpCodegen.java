...
public abstract class AbstractPhpCodegen extends DefaultCodegenConfig {
	...
	// composerVendorName/composerProjectName has be replaced by gitUserId/gitRepoId.
	...
	public AbstractPhpCodegen() {
		...
		setReservedWordsLowerCase(Arrays.asList(
				...
				"resourcePath", "httpBody", "queryParams", "headerParams", "formParams", "_header_accept", "_tempBody",
		...
		languageSpecificPrimitives = new HashSet<String>(Arrays.asList("bool", "boolean", "int", "integer", "double",
		...
		// cliOptions.add(new CliOption(COMPOSER_VENDOR_NAME, "The vendor name used in the
		// composer package name. The template uses
		// {{composerVendorName}}/{{composerProjectName}} for the composer package name.
		// e.g. yaypets. IMPORTANT NOTE (2016/03): composerVendorName will be deprecated
		// and replaced by gitUserId in the next swagger-codegen release"));
		cliOptions.add(new CliOption(CodegenConstants.GIT_USER_ID, CodegenConstants.GIT_USER_ID_DESC));
		// cliOptions.add(new CliOption(COMPOSER_PROJECT_NAME, "The project name used in
		// the composer package name. The template uses
		// {{composerVendorName}}/{{composerProjectName}} for the composer package name.
		// e.g. petstore-client. IMPORTANT NOTE (2016/03): composerProjectName will be
		// deprecated and replaced by gitRepoId in the next swagger-codegen release"));
	...
	public void processOpts() {
		...
		// this.setComposerProjectName((String)
		// additionalProperties.get(COMPOSER_PROJECT_NAME));
		// } else {
		...
		// this.setComposerVendorName((String)
		// additionalProperties.get(COMPOSER_VENDOR_NAME));
		// } else {
		...
		// // apache v2 license
		// supportingFiles.add(new SupportingFile("LICENSE", getPackagePath(),
		// "LICENSE"));
	}
	...
	public String toSrcPath(String packageName, String basePath) {
		packageName = packageName.replace(invokerPackage, ""); // FIXME: a parameter
																// should not be assigned.
																// Also declare the
																// methods parameters as
																// 'final'.
		if (basePath != null && basePath.length() > 0) {
			basePath = basePath.replaceAll("[\\\\/]?$", "") + File.separatorChar; // FIXME:
																					// a
																					// parameter
																					// should
																					// not
	...
	public String apiTestFileFolder() {
		return (outputFolder + File.separator + getPackagePath() + File.separator + testBasePath + File.separator
				+ apiDirName);
	}
	...
	public String toModelName(String name) {
		...
		name = name.replaceAll("[^\\w\\\\]+", "_"); // FIXME: a parameter should not be
	...
	public String toDefaultValue(Schema schema) {
		if (schema instanceof StringSchema) {
			...
			if (numberSchema.getDefault() != null && (SchemaTypeUtil.FLOAT_FORMAT.equals(schema.getFormat())
