...
public class JavaClientCodegen extends AbstractJavaCodegen
	...
	protected boolean doNotUseRx = true; // backwards compatibility for swagger configs
											// that specify neither rx1 nor rx2 (mustache
											// does not allow for boolean operators so we
											// need this extra field)
	...
	public void processOpts() {
		...
		final String authFolder = (sourceFolder + File.separator + invokerPackage + ".auth").replace(".",
				File.separator);
		...
		if ("feign".equals(getLibrary())) {
			...
			// the "okhttp-gson" library template requires "ApiCallback.mustache" for
			// async call
		...
		if (usesAnyRetrofitLibrary()) {
			...
			if (operations != null) {
				...
				for (CodegenOperation operation : ops) {
					...
					// sorting operation parameters to make sure path params are parsed
					// before query params
	...
	/**
	 * Prioritizes consumes mime-type list by moving json-vendor and json mime-types up
	 * front, but otherwise preserves original consumes definition order.
