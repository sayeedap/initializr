...
import io.swagger.codegen.v3.generators.features.CXFServerFeatures;
...
public class JavaCXFServerCodegen extends AbstractJavaJAXRSServerCodegen
	...
	public void processOpts() {
		...
		if (additionalProperties.containsKey(GENERATE_SPRING_APPLICATION)) {
			...
			this.setGenerateSpringBootApplication(
					convertPropertyToBooleanAndWriteBack(GENERATE_SPRING_BOOT_APPLICATION));
		}
		...
		if (additionalProperties.containsKey(GENERATE_NON_SPRING_APPLICATION)) {
			boolean generateNonSpringApplication = convertPropertyToBooleanAndWriteBack(
		...
		if (this.generateSpringApplication) {
			...
			writeOptional(outputFolder,
					new SupportingFile("server/web.mustache", ("src/main/webapp/WEB-INF"), "web.xml"));
			writeOptional(outputFolder,
					new SupportingFile("server/context.xml.mustache", ("src/main/webapp/WEB-INF"), "context.xml"));
		...
		if (this.generateNonSpringApplication) {
			writeOptional(outputFolder,
					new SupportingFile("server/nonspring-web.mustache", ("src/main/webapp/WEB-INF"), "web.xml"));
		}
	}
