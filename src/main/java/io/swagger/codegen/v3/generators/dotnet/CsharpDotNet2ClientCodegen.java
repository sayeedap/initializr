...
public class CsharpDotNet2ClientCodegen extends AbstractCSharpCodegen {
	...
	public CsharpDotNet2ClientCodegen() {
		...
		cliOptions.add(
				new CliOption(CodegenConstants.PACKAGE_VERSION, "C# package version.").defaultValue(packageVersion));
	...
	public void processOpts() {
		...
		supportingFiles.add(new SupportingFile("ApiClient.mustache", sourceFolder + File.separator + clientPackagePath,
				"ApiClient.cs"));
		...
		if (additionalProperties.containsKey(USE_CSPROJ_FILE)
				&& Boolean.parseBoolean(additionalProperties.get(USE_CSPROJ_FILE).toString())) {
