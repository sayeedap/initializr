...
public class RubyClientCodegen extends DefaultCodegenConfig {
	...
	public RubyClientCodegen() {
		...
		setReservedWordsLowerCase(Arrays.asList(
				...
				"ensure", "module", "redo", "super", "until", "BEGIN", "break", "do", "false", "next", "rescue", "then",
				"when", "END", "case", "else", "for", "nil", "retry", "true", "while", "alias", "class", "elsif", "if",
		...
		cliOptions
				.add(new CliOption(GEM_NAME, "gem name (convention: underscore_case).").defaultValue("openapi_client"));
		...
		cliOptions.add(new CliOption(GEM_REQUIRED_RUBY_VERSION, "gem required Ruby version. ").defaultValue(">= 1.9"));
		cliOptions.add(new CliOption(GEM_HOMEPAGE, "gem homepage. ").defaultValue("http://swagger.io"));
		cliOptions.add(new CliOption(GEM_SUMMARY, "gem summary. ").defaultValue("A ruby wrapper for the open APIs"));
		cliOptions.add(new CliOption(GEM_DESCRIPTION, "gem description. ").defaultValue("This gem maps to a open API"));
	...
	public void processOpts() {
		...
		writeOptional(outputFolder,
				new SupportingFile("configuration_spec.mustache", specFolder, "configuration_spec.rb"));
	...
	/**
	 * Generate Ruby module name from the gem name, e.g. use "SwaggerClient" for
	 * "swagger_client".
	 * @param gemName Ruby gem name
	 * @return Ruby module name
	 */
	private String generateModuleName(String gemName) {
		return camelize(gemName.replaceAll("[^\\w]+", "_"));
	}

	/**
	 * Generate Ruby gem name from the module name, e.g. use "swagger_client" for
	 * "SwaggerClient".
	 * @param moduleName Ruby module name
	 * @return Ruby gem name
	 */
	private String generateGemName(String moduleName) {
	...
	public String apiFileFolder() {
		return outputFolder + File.separator + libFolder + File.separator + gemName + File.separator
	...
	public String toApiFilename(String name) {
		...
		name = name.replaceAll("-", "_"); // FIXME: a parameter should not be assigned.
	...
	public void setParameterExampleValue(CodegenParameter p) {
		...
		else if (Boolean.TRUE.equals(p.getIsListContainer())) {
			example = "[" + example + "]";
		}
		else if (Boolean.TRUE.equals(p.getIsMapContainer())) {
			example = "{'key' => " + example + "}";
		}
