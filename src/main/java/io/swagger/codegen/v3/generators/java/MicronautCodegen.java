...
public class MicronautCodegen extends AbstractJavaCodegen implements BeanValidationFeatures, OptionalFeatures {
	...
	private void init() {
		...
		cliOptions.add(new CliOption(SKIP_SUPPORT_FILES,
				"skip support files such as pom.xml, mvnw, etc from code generation."));
	...
	public void processOpts() {
		...
		if (!skipSupportFiles) {
			...
			supportingFiles.add(new SupportingFile("unsupportedOperationExceptionHandler.mustache",
					...
					"UnsupportedOperationExceptionHandler.java"));
			supportingFiles.add(new SupportingFile("mainApplication.mustache",
					(sourceFolder + File.separator).replace(".", File.separator), "MainApplication.java"));
		}
	...
	private void addHandlebarsLambdas(Map<String, Object> objs) {
		Map<String, Lambda> lambdas = new ImmutableMap.Builder<String, Lambda>()
				...
				.put("capitalise", new CapitaliseLambda()).put("escapeDoubleQuote", new EscapeDoubleQuotesLambda())
				.put("removeLineBreak", new RemoveLineBreakLambda()).build();
